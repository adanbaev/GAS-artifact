...
public class IntegrityLogDao {
    ...
    // ---------------------------
    // CHECKPOINT verification reads
    // ---------------------------

    /**
     * Read checkpoint state without taking a write lock.
     * Used by post-hoc verification only.
     */
    public CheckpointState readCheckpointState() {
        return jdbcTemplate.queryForObject(
            "select last_checkpoint_hash, next_batch_no from integrity_checkpoint_state where id=1",
            (rs, rowNum) -> new CheckpointState(rs.getString(1), rs.getLong(2))
        );
    }

    /**
     * Read all finalized checkpoints in batch order.
     */
    public List<CheckpointRow> findAllCheckpointsOrdered() {
        return jdbcTemplate.query(
            "select id, batch_no, start_event_id, end_event_id, event_count, " +
                "root_hash, prev_checkpoint_hash, checkpoint_hash " +
                "from integrity_checkpoint order by batch_no asc, id asc",
            (rs, rowNum) -> new CheckpointRow(
                rs.getLong("id"),
                rs.getLong("batch_no"),
                rs.getLong("start_event_id"),
                rs.getLong("end_event_id"),
                rs.getInt("event_count"),
                rs.getString("root_hash"),
                rs.getString("prev_checkpoint_hash"),
    ...
    /**
     * Read the exact persisted event interval referenced by one checkpoint.
     */
    public List<CheckpointEventRow> findCheckpointEvents(long startEventId, long endEventId) {
        return jdbcTemplate.query(
            "select id, event_ms, actor_username, action, freq_id, data_hash " +
                "from freq_integrity_event where id between ? and ? order by id asc",
            (rs, rowNum) -> new CheckpointEventRow(
                rs.getLong("id"),
                rs.getLong("event_ms"),
                rs.getString("actor_username"),
                rs.getString("action"),
                rs.getObject("freq_id") == null ? null : rs.getLong("freq_id"),
    ...
    public static final class CheckpointRow {
        public final long id;
        public final long batchNo;
        public final long startEventId;
        public final long endEventId;
        public final int eventCount;
        public final String rootHash;
        public final String prevCheckpointHash;
        public final String checkpointHash;

        public CheckpointRow(long id, long batchNo, long startEventId, long endEventId,
                             int eventCount, String rootHash, String prevCheckpointHash,
            ...
            this.prevCheckpointHash = prevCheckpointHash;
            this.checkpointHash = checkpointHash;
        }
    }

    public static final class CheckpointEventRow {
        public final long id;
        public final long eventMs;
        public final String actorUsername;
        public final String action;
        ...
        public CheckpointEventRow(long id, long eventMs, String actorUsername,
                                  String action, Long freqId, String dataHash) {
            this.id = id;
            this.eventMs = eventMs;
            this.actorUsername = actorUsername;
            this.action = action;
